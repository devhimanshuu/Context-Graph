/**
 * ContextGraph — database seed (Phase 2)
 * =============================================================================
 * Seeds a realistic, domain-agnostic dataset:
 *
 *   2 organizations (healthcare + finance)  → proves multi-tenant isolation
 *   3 workspaces                            → knowledge containers
 *   department hierarchy (4 levels of depth) → org chart
 *   users across every role                 → permission model demo
 *   ~20 knowledge nodes + ~10 typed edges   → hospital assessment data,
 *                                             expressed generically
 *   permission profiles + assignments       → governance scaffolding
 *   context rules                           → deterministic rule engine input
 *   audit log events                        → event-sourcing foundation
 *
 * The healthcare assessment is expressed purely as DATA. Nothing here is
 * schema-specific — the same shapes serve finance, legal, education, etc.
 *
 * Idempotency: re-running wipes ONLY the two seeded organizations (and, via
 * database cascades, everything beneath them) before re-creating. It never
 * touches other data. In production, guard seeding behind an explicit flag —
 * `prisma db seed` is a developer/admin tool.
 * =============================================================================
 */
...
// Seed is a standalone dev/admin tool: it intentionally avoids the app's
// logging module (which is wired to Next's server-only env config) and writes
// to stdout directly.
