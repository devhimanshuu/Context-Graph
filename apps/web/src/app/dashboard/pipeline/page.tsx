...
import {
  ...
  RankedList,
  runRecordToPackage,
} from '@/components/dashboard/pipeline-run-detail'
import {
...
import type { ContextPackage, PipelineCandidate, PipelineRunRecord } from '@/lib/api/types'
...
/** Deep-link to a run's detail page from a diff stamp or column header. */
function RunStampLink({ run }: { run: PipelineRunRecord }) {
  return (
    <Link
      href={pipelineRunDetail(run.requestId)}
...
function RunHeaderLink({ run, label }: { run: PipelineRunRecord; label: string }) {
  return (
    <Link
      href={pipelineRunDetail(run.requestId)}
